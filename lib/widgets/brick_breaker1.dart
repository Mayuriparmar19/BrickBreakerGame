...
import 'package:dino_run/components/brick.dart';
...
class BricksBreaker extends FlameGame
  ...
  double get brickSize {
    const totalPadding = (numberOfBricksInOneRow + 1) * bricksPadding;
    final screenMinSize = size.x < size.y ? size.x : size.y;
    return (screenMinSize - totalPadding) / numberOfBricksInOneRow;
  }
  ...
  Future<void> loadInitialBrickLayer() async {
    for (var row = 0; row < numberOfBricksLayer; row++) {
      ...
      for (var i = 0; i < numberOfBricksInOneRow; i++) {
        final xPosition = i == 0
            ? 8
